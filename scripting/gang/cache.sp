...
stock void Gang_FillCache()
{
	...
	Format(sQuery, sizeof(sQuery), "SELECT GangID, GangName, Chat, Prefix, PrefixColor, MaxMembers FROM gang");
	SQL_TQuery(g_hDatabase, TQuery_Gang, sQuery, _, DBPrio_Low);
	
	Format(sQuery, sizeof(sQuery), "SELECT GangID, CommunityID, AccessLevel FROM gang_members");
	SQL_TQuery(g_hDatabase, TQuery_GangMembers, sQuery, _, DBPrio_Low);
...
public void TQuery_Gang(Handle owner, Handle hndl, const char[] error, any data)
{
	if (hndl != null)
	{
		while(SQL_FetchRow(hndl))
		{
			...
			iGang[bChat] = bool:SQL_FetchInt(hndl, 2);
			iGang[bPrefix] = bool:SQL_FetchInt(hndl, 3);
			SQL_FetchString(hndl, 4, iGang[sPrefixColor], 64);
			iGang[iMaxMembers] = SQL_FetchInt(hndl, 5);
			
			Log_File(_, _, DEBUG, "[TQuery_Gang] GangID: %d - GangName: %s - Chat: %d - Prefix: %d - PrefixColor: %s - MaxMembers: %d", iGang[iGangID], iGang[sGangName], iGang[bChat], iGang[bPrefix], iGang[sPrefixColor], iGang[iMaxMembers]);
...
public void TQuery_GangMembers(Handle owner, Handle hndl, const char[] error, any data)
{
	if (hndl != null)
	{
		while(SQL_FetchRow(hndl))
